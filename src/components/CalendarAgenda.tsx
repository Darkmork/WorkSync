...
import { CalendarAuthError, fetchTodayEvents, hasCalendarToken } from "../services/calendar";
...
export function CalendarAgenda() {
  ...
  return (
    <div className="rounded-xl border border-border-subtle bg-white p-6 shadow-soft">
      ...
      {status === "error" && (
        <button onClick={() => void load()} className="inline-flex items-center gap-2 rounded-lg border border-border-subtle px-4 py-2 text-sm font-bold text-primary">
          <RefreshCw size={16} />
          Reintentar
        </button>
      )}
        ...
        <ul className="space-y-2">
          ...
          {events.map((event) => (
            <li key={event.id} className="flex items-center gap-3 rounded-lg border border-border-subtle px-3 py-2">
              <span className="font-mono text-xs font-bold text-primary">{eventTime(event.start)}</span>
